use amethyst::{
    prelude::*,
    input::{InputHandler, InputEvent, VirtualKeyCode, StringBindings, ScrollDirection},
    winit::{MouseButton},
...
impl<'s> System<'s> for GodCameraSystem {
    type SystemData = (Read<'s, EventChannel<InputEvent<StringBindings>>>,
            ...
            match *event { 
                ...
                InputEvent::MouseMoved{ delta_x, delta_y } => { 
                    if input_manager.left_click_down {
                        transform.set_translation_xyz(transform.translation().x - delta_x, transform.translation().y + delta_y, 1.0);
                    }
                },
